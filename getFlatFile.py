...
lastIngestionDate=spark.sql("select coalesce(max(rovertimestamp), cast('2018-08-01' as timestamp)) as lastIngestionDate from parquet.ingestion_tracker where source_name= 'pdsdataextraction' ").first()["lastIngestionDate"]
